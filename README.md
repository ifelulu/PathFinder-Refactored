...
*   **Pathfinding:**
    *   **Grid Representation:** Generates a configurable 2D grid representation of the layout, mapping obstacles and staging area penalties.
...
*   **Project Management:**
    *   **Save/Load:** Save the entire project state (PDF reference, scale, all layout definitions, settings) to a JSON-based project file (`.whp`) and load it back.
...
4.  **Tools > Set Pick Aisle / Set Staging Location**: Define named start and end points for paths. (Use Line Definition tools for multiple points).
5.  **Tools > Precompute All Paths**: This is crucial for analysis and animation. It calculates paths from all Pick Aisles. Run this after any layout changes.
6.  **Tools > Calculate Path**: Select a start/end pair to see a single shortest path.
7.  **Tools > Analyze Picklist...**: Load a CSV, map columns, and view distance statistics.
8.  **Tools > Animate Picklist...**: Load a timed CSV to visualize warehouse activity.
9.  **File > Save Project / Save Project As...**: Save your work.
...
*   **Pathfinding Problems:**
    ...
    *   Run "Precompute All Paths" after any layout changes (obstacles, points, scale, grid factor, staging penalty).
